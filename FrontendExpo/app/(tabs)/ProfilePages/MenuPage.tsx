...
export default function Menupage({ navigation }) {
  ...
  const fetchInfo = async () => {
    try {
      ...
      const url = `https://consumer-corner.kvotua.ru/profile/docs_socials_exist`;
    ...
    <ImageBackground source={require("../../../assets/images/background.png")} style={styles.background}>
      <SafeAreaView style={styles.containerMainPage}>
        <View style={[styles.menuHeader, { marginTop: Platform.OS === 'ios' ? 10 : 50, height: 80, justifyContent: 'flex-end', }]}>
          <Text style={styles.menuTitle} >Здравствуйте, {info.name}</Text>
        </View>

        <View style={styles.menuTopButtons}>
          <TouchableOpacity
            style={info.documents ? styles.whiteButtonMenuTopActive : styles.whiteButtonMenuTopNoActive}
          ...
          >
            <View style={{ flex: 1, justifyContent: "space-between" }}>
              ...
              <Image
                ...
                style={{ width: 25, height: 25, alignSelf: "flex-end", bottom: info.documents ? 10 : 20, right: 10 }}
              />
            </View>
          </TouchableOpacity>
          <TouchableOpacity
            style={info.socials ? styles.whiteButtonMenuTopActive : styles.whiteButtonMenuTopNoActive}
            onPress={() => navigation.replace("Social")}
          >
            <View style={{ flex: 1, justifyContent: "space-between" }}>
              <Text style={[styles.blackText, { paddingTop: isTablet ? 30 : 25, paddingStart: 15 }]}>Соц. сети</Text>
              {!info.socials && (
                <Text style={[styles.alertText, { paddingStart: 15 }]}>Не заполнены</Text>
              )}
              <Image
                source={require("../../../assets/images/corner.png")}
                style={{ width: 25, height: 25, alignSelf: "flex-end", bottom: info.socials ? 10 : 20, right: 10 }}
              />
            </View>
          </TouchableOpacity>
          {/* <TouchableOpacity
            style={styles.whiteButtonMenuTopActive}
            onPress={() => navigation.replace("Social")}
          >
          <View style={[{ flex: 1, justifyContent: "space-between", }]}>
            <Text style={[styles.blackText, { paddingTop: isTablet ? 30 : 25, paddingStart: 15 }]}>Соц. сети</Text>
          ...
          >
            <View style={[{ flex: 1, justifyContent: "space-between", }]}>
              ...
              <View style={styles.containerBell}>
                {info.comments > 0 && (
                  <>
                    <Text style={[{ fontSize: 10, fontWeight: "bold", fontFamily: "Montserrat", textAlign: "right", top: 4 }]}>
                      {info.comments}
                    </Text>
